...
def road_init_with_fit(depth, vis=1, fit=1, normal_threshold=0.2,upper_fit_thred=0.02,lower_fit_thred=0.1,focal_length=500):
    ...
    if not fit:
        ...
        if len(specific_points)<=3:
            return np.zeros(depth.shape, dtype=np.uint8),0
        else:
